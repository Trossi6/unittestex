package animals.petstore.pet.attributes;

/**
 * Breed is an enum for Dogs, Cats, Birds, and Snakes that can be found in the pet store
 */
public enum Breed {
    POODLE, MALTESE, GOLDEN_DOODLE, GREY_HOUND, GERMAN_SHEPARD, //DOGS
    SIAMESE, MAINE, BIRMAN, RAGDOLL, BURMESE, SPHYNX, //CATS
    BLUE_JAY, CARDINAL, SPARROW, HAWK, HUMMING_BIRD, ROBIN, BALTIMORE_ORIOLE, //Birds
    BALL_PYTHON, BURMESE_PYTHON, COPPERHEAD, GRASS, MILK, CORN, CORAL, //SNAKES
    PARROT, EAGLE, UNKNOWN
}
